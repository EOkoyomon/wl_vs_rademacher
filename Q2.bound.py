    ...
    for l in num_layers:
        ...
        for it in range(num_reps):
            ...
            def calculate_rademacher_complexity(model, train_loader, m):
                ...
                B1, B2 = max_l2_norms_weights(model)
                ...
                # R <= C_rho * C_g * d * min{b*sqrt(r), B1*B_x*M}
                b = 1.0
                R = C_rho * C_g * d * min(b * torch.sqrt(torch.tensor(r)), B1 * B_x * M)
                ...
                # Q = 24 * B_beta * sqrt(m) * max{Z, M*sqrt(r)*max{B_x*B1, R*B2}}
                Q = 24 * B_beta * torch.sqrt(torch.tensor(m)) * max(Z, M * torch.sqrt(torch.tensor(r)) * max(B_x * B1, R * B2))
                
                # R^T(J_gamma) <= 4/(gamma*m) + (24*r*B_beta*Z)/(gamma*sqrt(m)) * sqrt(3*log(Q))
