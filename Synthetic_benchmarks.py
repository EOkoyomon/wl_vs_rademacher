...
def perturb_graph(G_original, num_steps, perturbation_type):
    """
    Given an original graph G_original, perform num_steps random local perturbations.
    The type of perturbation is determined by perturbation_type, which can be:
    ...
    for _ in range(num_steps):
        if perturbation_type == 'edge_rewire':
            edges = list(G.edges())
            nodes = list(G.nodes())
        ...
        elif perturbation_type == 'edge_add':
        ...
        elif perturbation_type == 'edge_remove':
        ...
        elif perturbation_type == 'node_remove':
    ...
    def process(self):
        ...
        for i in range(self.num_graphs):
            if i < sample_to_perturb: #if sample_to_perturb == self.num_graphs, all samples are perturbed
                G_p = perturb_graph(base_graphs[i], self.K, self.perturbation_type)
            else: # Perturb only a fraction of the samples and ensures monotonicity for larger \rho values
                G_p = perturb_graph(base_graphs[i], 0, self.perturbation_type) # No perturbation
        ...
        csv_file = os.path.join(self.root, "wl_statistics_memorization.csv")
...
class ParentClassificationDataset(InMemoryDataset):
    def __init__(self, root, parent_A, parent_B, num_graphs_per_class=10,
        ...
        self.degree = list(parent_A.degree())[0][1] # Assuming regular graphs
        self.num_graphs_per_class = num_graphs_per_class
        self.regime = regime # 'all' or 'fraction'
        self.K = K # Number of rewires
        self.rho = rho # Fraction of samples to perturb
        self.perturbation_type = perturbation_type
    ...
    def processed_file_names(self):
        if self.regime == 'all':
            return [f'S_all_K{self.K}_N{self.total_graphs}_n{self.nodes}_deg{self.degree}_P-{self.perturbation_type}_seed{self.seed}.pt']
        elif self.regime == 'fraction':
            return [f'S_rho{self.rho:.2f}__K{self.K}_N{self.total_graphs}_n{self.nodes}_deg{self.degree}_P-{self.perturbation_type}_seed{self.seed}.pt']
    ...
    def process(self):
        ...
        if self.regime == 'all':
            sample_to_perturb = self.num_graphs_per_class
        elif self.regime == 'fraction':
            sample_to_perturb = int(self.rho * self.num_graphs_per_class)

        base_graphs_A = [self.parent_A.copy() for _ in range(self.num_graphs_per_class)] 
        base_graphs_B = [self.parent_B.copy() for _ in range(self.num_graphs_per_class)]
        
### Generation of children graphs by perturbing A, class +1
        for i in range(self.num_graphs_per_class):
            applied_K = self.K if i < sample_to_perturb else 0
            G_p = perturb_graph(base_graphs_A[i], applied_K, self.perturbation_type)

            # computing the wl-hashing for the perturbed graph
            h = nx.weisfeiler_lehman_graph_hash(G_p, iterations=self.wl_iter)
            wl_hashes_A.append(h)

            data = from_networkx(G_p)
            data.x = torch.ones((data.num_nodes, 1)) # Feature all 1
            data.y = torch.tensor([1], dtype=torch.long) # Label for class +1
            # Metadata saved in the graph data object
            data.K = torch.tensor([self.K], dtype=torch.long) 
            data.rho = torch.tensor([self.rho], dtype=torch.float)
            data.wl_hash = h # Save the WL hash as a string attribute (not a tensor)
...
### Generation of children graphs by perturbing B, class -1
        for i in range(self.num_graphs_per_class):
            applied_K = self.K if i < sample_to_perturb else 0
            G_p = perturb_graph(base_graphs_B[i], applied_K, self.perturbation_type)

            h = nx.weisfeiler_lehman_graph_hash(G_p, iterations=self.wl_iter)
            wl_hashes_B.append(h)
        ...
        # print wl statistics for this K
        wl_hashes = wl_hashes_A + wl_hashes_B
        unique_classes = len(set(wl_hashes))
        unique_classes_A = len(set(wl_hashes_A))
        unique_classes_B = len(set(wl_hashes_B))

        print(f"-> Regime: {self.regime.upper()} | K: {self.K} | Rho: {self.rho} | Type: {self.perturbation_type}")
        print(f"-> WL Equivalence Classes (p): {unique_classes} / {self.total_graphs}")
        print(f"-> WL Equivalence Classes (A): {unique_classes_A} / {self.total_graphs}")
        print(f"-> WL Equivalence Classes (B): {unique_classes_B} / {self.total_graphs}")

        print("-" * 60)

        csv_file = os.path.join(self.root, "wl_statistics_parents.csv")
        file_exists = os.path.isfile(csv_file)
        ...
        with open(csv_file, mode="a", newline="") as f:
          ...
          #################controlla####################  
            if not file_exists:
                writer.writerow(["Num_Graphs", "Nodes", "Degree","Regime", "K", "Rho", "Perturbation_Type", "WL_Classes","WL_Classes_A", "WL_Classes_B"])
...
if __name__ == "__main__":
    ...
    for r in [0.1, 0.4, 0.8]:
        dataset = ParentClassificationDataset(root=root, parent_A=parent_A, parent_B=parent_B, regime='fraction', K=3, rho=r)
